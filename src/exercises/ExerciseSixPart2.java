 ...
 * @author aalvarado
 */
public class ExerciseSixPart2 {

    public static void main(String[] args) {
        String data, action, from, to;
        int fromRow, fromColumn, toRow, toColumn;
        Scanner scanner = new Scanner(System.in);
        int[][] ligths = new int[1000][1000];
        ...
        do {
            ...
            action = strToken.nextToken();
            from = strToken.nextToken();
            strToken.nextToken();
            to = strToken.nextToken();

            fromRow = Integer.parseInt(from.substring(0, from.indexOf(",")));
            fromColumn = Integer.parseInt(from.substring(from.indexOf(",") + 1));

            toRow = Integer.parseInt(to.substring(0, to.indexOf(",")));
            toColumn = Integer.parseInt(to.substring(to.indexOf(",") + 1));

            for (int row = fromRow; row <= toRow; row++) {
                for (int column = fromColumn; column <= toColumn; column++) {
                    if (action.equals("on")) {
                        ligths[row][column]++;
                    } else if (action.equals("off") && ligths[row][column] > 0) {
                        ligths[row][column]--;
                    } else if (action.equals("toggle")) {
                        ligths[row][column] = ligths[row][column] + 2;
                    }
                }
            }
            ...
            System.out.println("Ligths On: " + sum);
